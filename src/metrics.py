...
class Metrics:
    ...
    def _get_tokenizer(self):
        if self.gen_ppl_eval_model_name_or_path is None:
            raise ValueError("Set eval.gen_ppl_eval_model_name_or_path to compute PPL.")
    ...
    def _get_eval_model(self, device):
        ...
        if self._eval_model is None:
            self._eval_model = transformers.AutoModelForCausalLM.from_pretrained(
                self.gen_ppl_eval_model_name_or_path,
                local_files_only=True,
            ).eval()
            self._eval_model_device = "cpu"
        if "llama2" not in self.gen_ppl_eval_model_name_or_path:
            target_device = str(device)
            if self._eval_model_device != target_device:
                self._eval_model = self._eval_model.to(device)
                self._eval_model_device = target_device
...
def compute_mauve_score(
    generated_texts: typing.Sequence[str],
    reference_texts: typing.Sequence[str],
    featurize_model_name_or_path: str,
    device_id: int,
    max_text_length: typing.Optional[int] = None,
    ...
    try:
        ...
        raise ImportError(
            "Missing dependency for MAUVE. Install with `pip install mauve-text`."
        ) from exc

    if not generated_texts:
        raise ValueError("Generated text list is empty.")
    if not reference_texts:
        raise ValueError("Reference text list is empty.")

    mauve_kwargs = {
        "p_text": list(generated_texts),
        "q_text": list(reference_texts),
        "device_id": device_id,
        "featurize_model_name": featurize_model_name_or_path,
        "verbose": verbose,
    }
    if max_text_length is not None:
        mauve_kwargs["max_text_length"] = max_text_length
    if num_buckets is not None:
        mauve_kwargs["num_buckets"] = num_buckets
    return mauve.compute_mauve(**mauve_kwargs)
