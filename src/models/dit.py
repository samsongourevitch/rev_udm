...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, config, vocab_size: int):
        ...
        self.condition_on_uniform_noise = bool(
            getattr(config.algo, "condition_on_uniform_noise", False)
        )
        self.conditioning_fusion = getattr(
            config.algo, "conditioning_fusion", "separate_linear"
        )
        if self.condition_on_uniform_noise:
            if self.conditioning_fusion == "separate_linear":
                self.conditioning_vocab_embed = EmbeddingLayer(dim, vocab_size)
                self.conditioning_input_proj = nn.Linear(2 * dim + 1, dim)
            elif self.conditioning_fusion == "shared_mlp":
                self.conditioning_input_mlp = nn.Sequential(
                    nn.Linear(2 * dim + 1, 4 * dim),
                    nn.GELU(approximate="tanh"),
                    nn.Linear(4 * dim, dim),
                )
            else:
                raise ValueError(
                    "Unsupported algo.conditioning_fusion: "
                    f"{self.conditioning_fusion}"
                )
        if not self.causal:
    ...
    def forward(self, x, sigma, conditioning_tokens=None, **kwargs):
        if x.ndim == 2:
            xt_tokens = x
        else:
            xt_tokens = x.argmax(dim=-1)
        x = self.vocab_embed(x)
        if self.condition_on_uniform_noise:
            if conditioning_tokens is None:
                raise ValueError(
                    "Model configured to receive noise conditioning "
                    "but the conditioning noise is None"
                )
            matches = (xt_tokens == conditioning_tokens).to(x.dtype)
            if self.conditioning_fusion == "separate_linear":
                conditioning = self.conditioning_vocab_embed(conditioning_tokens)
                x = self.conditioning_input_proj(
                    torch.cat([x, conditioning, matches.unsqueeze(-1)], dim=-1)
                )
            else:
                conditioning = self.vocab_embed(conditioning_tokens)
                x = self.conditioning_input_mlp(
                    torch.cat([x, conditioning, matches.unsqueeze(-1)], dim=-1)
                )
        if self.causal:
            t_cond = None
