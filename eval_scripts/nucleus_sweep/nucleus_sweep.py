...
python3 eval_scripts/nucleus_sweep/nucleus_sweep.py prepare --run-name nucleus_eval_foo
python3 eval_scripts/nucleus_sweep/nucleus_sweep.py launch --run-name nucleus_eval_foo
python3 eval_scripts/nucleus_sweep/nucleus_sweep.py finalize --run-name nucleus_eval_foo
"""

REPO_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_CONFIG = Path("configs/sweeps/nucleus_eval_manifest.yaml")


def nucleus_tag(p_nucleus: float) -> str:
    return f"{p_nucleus:.2f}".replace(".", "p")
...
def manifest_id_for_row(entry_idx: int, row, p_nucleus: float) -> str:
    base_id = row.get("id")
    if base_id is None:
        ...
        return (
            ...
            f"pnucleus{nucleus_tag(float(p_nucleus))}"
        )
    return f"{base_id}_pnucleus{nucleus_tag(float(p_nucleus))}"
...
def build_manifest_rows(cfg):
    ...
    for entry_idx, entry in enumerate(cfg.entries):
        ...
        base_row = {
            ...
            "seeds": seeds,
            "nfes": nfes,
            "temperature": float(cfg.sweep.temperature),
        ...
        for p_nucleus in cfg.sweep.p_nucleus_list:
            row = dict(base_row)
            row["p_nucleus"] = float(p_nucleus)
            row["id"] = manifest_id_for_row(entry_idx, row, float(p_nucleus))
            rows.append(row)
...
def cmd_launch(args):
    ...
    slurm_script = (
        ...
        / "bash_scripts"
        / "run_nucleus_manifest_slurm_array.sh"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Nucleus sweep workflow.")
    sub = parser.add_subparsers(dest="cmd", required=True)

    prepare = sub.add_parser("prepare", help="Create or reuse manifest.jsonl, then filter pending rows.")
    ...
    launch = sub.add_parser("launch", help="Submit the pending manifest as a Slurm array.")
    ...
    finalize = sub.add_parser("finalize", help="Merge run outputs if needed.")
