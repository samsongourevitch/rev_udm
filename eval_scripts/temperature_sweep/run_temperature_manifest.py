...
def visible_devices_string(primary_token: str, all_tokens):
    ...
    ordered.extend(
        str(token) for token in all_tokens if str(token) != str(primary_token)
    )
    return ",".join(ordered)
...
def run_single_eval(
    ...
    sample_filename = (
        ...
        f"nfe{nfe}_temp{temp_tag(temperature)}_"
        f"tempout-{str(bool(apply_temp_output)).lower()}_seed{seed}.json"
    )
...
@hydra.main(version_base=None, config_path=CONFIG_DIR, config_name="temp_eval_manifest")
def main(cfg):
    ...
    seeds = row.get("seeds", cfg.sweep.seeds)
    nfes = row.get("nfes", cfg.sweep.nfe_list)
    seeds = [int(seed) for seed in seeds]
    nfes = [int(step) for step in nfes]
    ...
    num_sample_batches = int(
        row.get("num_sample_batches", cfg.sweep.num_sample_batches)
    )
    ...
    if not gpu_tokens:
        raise ValueError("No GPUs available for temperature sweep execution")
    ...
    status_payload = write_status_files(entry_dir, all_rows, expected_count)
    print(f"Saved entry results to: {entry_dir}")
    if not status_payload["complete"]:
        raise SystemExit(
            "Incomplete run for "
            f"{manifest_id}: expected {expected_count} successful results, "
            f"got {status_payload['produced']} records "
            f"({status_payload['success']} success, {status_payload['failed']} failed)"
        )
